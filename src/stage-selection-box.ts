...
export function syncStageSelectionBox(input: {
  ...
  const {activeLabelNode, projector, selectionBox, textLayer, viewport} = input;
  ...
  const bounds = textLayer.getLabelScreenBounds(activeLabelNode.label, projector, viewport);
